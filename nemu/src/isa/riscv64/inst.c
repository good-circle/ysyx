...
static void decode_operand(Decode *s, word_t *dest, word_t *src1, word_t *src2, int type) {
  ...
  switch (type) {
    ...
    case TYPE_J: destR(1); src1J(immJ(i)); break;
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4; s->dnpc = (src1 + src2) | 0);

  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = src1 < src2);

  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (sword_t)src1 >> ((src2 << 58) >> 58));

  INSTPAT("??????? ????? ????? 011 ????? 00000 11", ld     , I, R(dest) = Mr(src1 + src2, 8));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = (sword_t)((int32_t)Mr(src1 + src2, 4)));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = (word_t)((uint8_t)Mr(src1 + src2, 1)));
