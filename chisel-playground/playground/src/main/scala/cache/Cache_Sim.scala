...
class Cache_Sim extends Module with Config{
  ...
  (sram_rdata zip sram).map { case (r, s) => r := s.io.rdata}
    ...
    is (refill) {
      ...
      for (i <- 0 until 4) {
        when (replace_way === i.U && out.ret_valid && !rb_uncache) {
          ...
          sram(i).io.addr := rb_index
          when (rb_op === WRITE) {
            sram(i).io.wdata := Mux(rb_offset(3),
    ...
    is (wb_write) {
      ...
      for (i <- 0 until 4) {
        when (wb_hitway === i.U) {
          sram(i).io.en := true.B
          sram(i).io.wen := true.B
          sram(i).io.addr := wb_index
          sram(i).io.wdata := Mux(wb_offset(3),
